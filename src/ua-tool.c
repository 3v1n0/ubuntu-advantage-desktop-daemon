...
static void ua_attach_call_client(GObject *object, GAsyncResult *result,
  ...
  // We don't really care about of the result of this operation.
  g_output_stream_close_async(output_stream, G_PRIORITY_DEFAULT, NULL, NULL, NULL);

  attach_data = g_task_get_task_data(task);
  subprocess = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_PIPE, &error,
      "pro", "attach", "--attach-config", g_file_peek_path(attach_data->config_file), NULL);
  if (subprocess == NULL) {
    g_task_return_error(task, g_steal_pointer(&error));
    return;
  }
...
static void write_attach_config_file(GFile *config_file, GFileIOStream *iostream,
                                     GTask *task) {
  ...
  attach_data = g_new0(AttachData, 1);
  // See: https://canonical-ubuntu-pro-client.readthedocs-hosted.com/en/latest/howtoguides/how_to_attach_with_config_file/
  ...
  g_output_stream_write_all_async(output_stream, attach_data->config_contents,
                                  strlen(attach_data->config_contents),
                                  G_PRIORITY_DEFAULT, cancellable,
                                  ua_attach_call_client,
                                  g_steal_pointer(&task));
}
...
static void ua_token_file_create_cb(GObject *object, GAsyncResult *result,
                                    gpointer user_data) {
  g_autoptr(GFile) config_file = G_FILE(object);
  g_autoptr(GTask) task = G_TASK(user_data);
  g_autoptr(GFileIOStream) iostream = NULL;
  g_autoptr(GError) error = NULL;

  config_file = g_file_new_tmp_finish(result, &iostream, &error);
  if (!config_file) {
    g_task_return_error(task, g_steal_pointer(&error));
    return;
  }
